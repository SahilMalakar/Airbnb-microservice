...
export async function findActiveHold(hotelId: number, tx: Prisma.TransactionClient) {
...
export async function confirmBookingWithLock(bookingId: number, tx: Prisma.TransactionClient) {
    ...
    if (result.count === 0) {
        ...
        if (booking?.status === "PENDING" && booking.holdExpiresAt <= new Date()) {
            await tx.booking.update({
                where: { id: bookingId },
                data: { status: "EXPIRED" },
            });
            throw new BadRequestError("Booking hold has expired, please book again");
        }
